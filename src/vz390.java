...
//                All lines containing '!!' should be reviewed before deployment.   HS


/**
This class services all VSAM requests.

Majorly restructured at RPI 1614. It now supports the original zVSAM logic (version 1) as well 
as the revised and extended logic (version 2). (Informally called dual-version framework.)

Original source class structure:
...
Source class structure after RPI 1614: Two inner classes have been defined. V1 contains all the
methods from the original vz390. All the fields from the original vz390 remain in vz390. Class
V2 contains <em>all</em> the methods required for the new logic, and any fields that are needed
additional to the original fields. All the original fields in class vz390 are accessible to each
of the inner classes. The contents of the two inner classes are totally opaque to each other,
though visible to the containing class.
...
This dual-version framework is intended to continue to support the original VSAM implementation
during the (however long) development of version 2. The way that this works is that a runtime
parameter (zVSAM(n)) will specify which version is to run. Values of n can be 1 or 2 (default 1).
<p>
A constructor for vz390 was introduced at RPI 1614. It chooses, dependent on the runtime parm, to 
create a V1 object <em>or</em> a V2 object, but not both. The one remaining original method 
(named svc_vsam) in vz390 will simply forward each VSAM Request to the method named 
handle_vsam_request in the created object.
<p>
After initialisation, one of the following sets of objects exist, dependent on version chosen:
<pre>

  if parm ZVSAM(1) was specified (or defaulted):

                                   +-------------------------------------------------+
                                   | Object name: vz390                              |
                                   | - method name: svc_vsam                         |
                                   |    +---------------------------------------+    |
                                   |    | Object name: the_VSAM_Handler         |    |
                                   |    | Object type: V1                       |    |
...
By virtue of implementing the same interface, <code>Generic_VSAM_Handler</code>, either object can be referenced
by <code>the_VSAM_Handler</code>. And implementing that interface guarantees that the JVM will be able to resolve
the method name <code>handle_vsam_request</code> no matter which of the two classes has been instantiated.
*/
...
public class vz390 {
    ...
    * 03/09/18 RPI 1614 Re-structure into V1 & V2. HS
    ...
    **************************************************************************/
     ...
     final byte vsam_op_get   =  1;
     final byte vsam_op_put   =  2;
     final byte vsam_op_erase =  3;
     final byte vsam_op_point =  4;
     final byte vsam_op_open  = 19;
     final byte vsam_op_close = 20;
     ...
     /**
      * init logical reason codes for use in set_feedback tracing
      */
     ...
     /*
      * init physical reason codes for use in set_feedback tracing
      */
...
<pre>
   ...
   interface Generic_VSAM_Handler {              // RPI 1614
     ...
     public byte isValidACB(int addr);
   }
   ...
   */
   ...
   /**
    *  This method forwards the request to whichever handler has been instantiated.
    */
   public void svc_vsam() {                      // RPI 1614
     the_VSAM_Handler.handle_vsam_request(cur_vsam_op);     // Forward it to the chosen handler.
   }
...
// =========================== End of outer class members. ===========================



/** The original (V1) zVSAM implementation. */

   private class V1 implements Generic_VSAM_Handler {  // RPI 1614
   ...
   */
   public byte isValidACB(int acb_addr) {
     if (isValidV1ACB(acb_addr)) return (byte) 0;    // valid
   ...
   /** 
    ...
    /*
     * Close open acb.
     * 1. Close VESDCB 2. Close VX0 if not ESDS or fixed RRDS
     ...
     * 1. If file spec includes dot, use suffix to find VCDT entry 
     *    else use ACBNAME field. 
   ...
   /**
     ...
     * cur_vx0_xrba = cur_rpl_cxrba. 
     * 2. Set cur_ves_xrba as follows if cur_vx0_xrba > eod set 
     *    cur_vx0_xrba = -1 (not found) and return false
